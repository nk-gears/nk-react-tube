  ...
  return (
    <div className="videoPlayer">
      <div className="embed-responsive embed-responsive-16by9">
        <iframe
          title={playVideo.id.videoId}
          className="embed-responsive-item"
          src={"https://youtube.com/embed/" + playVideo.id.videoId}
          allowFullScreen
        >
          {" "}
        </iframe>{" "}
      </div>{" "}
      <div> {playVideo.snippet.title} </div> <div> {publishedOn} </div>{" "}
      <a
        href={"https://youtube.com/channel/" + playVideo.snippet.channelId}
