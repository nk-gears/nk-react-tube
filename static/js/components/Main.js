...
class Main extends Component {
  ...
  componentDidMount() {
    //Youtube Endpoint
    getYoutubeContentMain(
      this,
      "https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=12&type=video&order=viewCount&q=ReactJS%20Tutorial&key=" +
    ...
    if (this.state.isLoaded && this.state.playVideo === false) {
      return (
        <div className="container-fluid Main">
          <div className="row">
            {" "}
            {vidArr.slice(0, 3).map((i) => (
              <div className="col-md-4" key={i.etag}>
                {" "}
                <div onClick={() => this.clickHandler(i)} key={i.etag}>
                  {" "}
                  <Videos videoItem={i} key={i.etag} />
                </div>{" "}
              </div>
            ))}{" "}
          </div>{" "}
